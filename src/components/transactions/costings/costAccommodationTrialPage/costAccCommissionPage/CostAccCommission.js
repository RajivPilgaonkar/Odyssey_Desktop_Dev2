...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetAccCommissionData";
...
class CostAccCommission extends React.Component {
  _isMounted = false;
  ...
  fetchInitialData = async() => {
    ...
    if (this.state.isDataFetched) {
      ...
      if (this.props.onCostAccCommissionLoad !== undefined) {
        this.props.onCostAccCommissionLoad(false);
      }  
    }
    ...
    if (this._isMounted) {
      ...
      if (this.props.onCostAccCommissionLoad !== undefined) {
        this.props.onCostAccCommissionLoad(true);
      }
    }
  ...
  onFocusedRowChanged = async (e) => {
    ...
    if (this.props.onChangeCommission !== undefined) {
      await this.props.onChangeCommission(e);
    }
  }
  ...
  renderContent() {
    ...
    console.log('in commission');

    return (
      <div style={{width: '100%'}}>
        <ToolbarOptions text={"Commission"} {...accCommissionProps} ></ToolbarOptions>
...
export default connect(mapStateToProps,mapDispatchToProps)(withRouter(CostAccCommission));
