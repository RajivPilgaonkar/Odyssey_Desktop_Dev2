...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetSrvClosedData";
...
class CostSrvClosed extends React.Component {
  _isMounted = false;
  ...
  constructor(props) {
    ...
    this.var = { dataGridRef: React.createRef(), 
      mainData: [], tableName: 'CostServicesClose', keyField: 'CostServicesClose_id',
      userLookup: [],
  ...
  fetchInitialData = async() => {
    ...
    this.var.userLookup = await dbGetRecord({fields: ['AdmUsers_id', 'uid'], orders: ['uid'], table: 'admUsers', file: 'CostSrvClosed'});

    await this.filterData();

    if (this._isMounted) {
      this.setState({isDataFetched: true}, async () => {
        await this.props.onCostSrvClosedLoad(true);
      });   
    }
  ...
  filterData = async () => {
    ...
    const costservices_id = (this.props.costservices_id !== undefined) ? this.props.costservices_id : -1;

    const whereStr = 'costservices_id = ' + costservices_id.toString();
    this.var.mainData = await dbGetRecord({fields: fieldArray, orders: ['FromDate,ToDate'], table: this.var.tableName, where: whereStr, file: 'CostSrvClosed'});
  ...
  editRow = async (e) => {
    ...
    const agentObj = await getAgentFromCostServices(costservices_id);
  ...
  addRow = async () => {
    ...
    this.var.formMode = 1;
    this.var.formTitle = 'New closed period for ' + agentObj.orgService;
  ...
  saveFormData = async () => {
    ...
    let condition = "WHERE costservices_id = " + this.var.formData.CostServices_id + " " + 
      "AND FromDate = '" + moment(this.var.formData.FromDate).format('MM/DD/YYYY') + "' ";
    condition += (this.var.formMode === 2) ? "AND CostServicesClose_id <> " + this.var.formData.CostServicesClose_id: "";
  ...
  onFocusedRowChanged = async (e) => {
    ...
    if (this.props.onChangeClosedRow !== undefined) {
      await this.props.onChangeClosedRow(e);
    }
  }
  ...
  renderContent() {
    ...
    const dataObj = {
      ...
      canAddRow: false,
      //canDeleteRow: false,
      addRowText: 'Add a Closed Date Range',      
    ...
    const srvClosedProps = {
      numButtons: 1,
      buttonListObj: [
      {visible: canAdd, options: {icon: "add", onClick: this.addRow, hint: 'Add a new closed period'}}
      ],
      height: 40
    };
    ...
    return (
      <div style={{width: '100%', height: '100%'}}>
        <ToolbarOptions text={"Closed On"} {...srvClosedProps} ></ToolbarOptions>
...
export default connect(mapStateToProps,mapDispatchToProps)(withRouter(CostSrvClosed));
