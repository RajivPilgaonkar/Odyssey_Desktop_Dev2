...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetSrvGuideData";
...
class CostSrvGuide extends React.Component {
  _isMounted = false;
  ...
  constructor(props) {
    ...
    this.var = { dataGridRef: React.createRef(), 
      mainData: [], tableName: 'costservicesguides', keyField: 'costservicesguides_id',
  ...
  fetchInitialData = async() => {
    ...
    if (this._isMounted) {
      this.setState({isDataFetched: true}, async() => {
        await this.props.onCostSrvGuideLoad(true);
      });   
    }
  ...
  componentDidUpdate = async (prevProps) => {
    ...
    if (this.props.costservices_id !== prevProps.costservices_id) {      
      ...
      if (this.props.onCostSrvGuideLoad !== undefined) {
        this.props.onCostSrvGuideLoad(true);
      }
  ...
  addRow = async () => {
    ...
    this.var.formMode = 1;
    this.var.formTitle = 'New guide cost for ' + agentObj.orgService;
  ...
  saveFormData = async () => {
    ...
    condition += (this.var.formMode === 2) ? "AND costservicesguides_id <> " + this.var.formData.costservicesguides_id: "";
  ...
  onFocusedRowChanged = async (e) => {
    this.setState({costservicesguides_id: e.row.data.costservicesguides_id});
    if (this.props.onChangeGuideRow !== undefined) {
      await this.props.onChangeGuideRow(e);
    }
  }
  ...
  renderContent() {
    ...
    const srvGuideProps = {
      numButtons: 1,
      buttonListObj: [
      {visible: canAdd, options: {icon: "add", onClick: this.addRow, hint: 'Add a new Guide cost'}}
      ],
      height: 40
    };
    ...
    return (
      <div style={{width: '100%', height: '100%'}}>
        <ToolbarOptions text={"Guide Costs"} {...srvGuideProps} ></ToolbarOptions>
...
export default connect(mapStateToProps,mapDispatchToProps)(withRouter(CostSrvGuide));
