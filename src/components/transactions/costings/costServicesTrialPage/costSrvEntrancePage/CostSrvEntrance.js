...
import { getDevExtremeTable, getDevExtremePopupForm, tableHeaderArray } from "./GetSrvEntranceData";
...
class CostSrvEntrance extends React.Component {
  _isMounted = false;
  ...
  constructor(props) {
    ...
    this.var = { dataGridRef: React.createRef(), 
      mainData: [], tableName: 'costservicesentrancefees', keyField: 'costservicesentrancefees_id',
  ...
  fetchInitialData = async() => {
    ...
    if (this._isMounted) {
      this.setState({isDataFetched: true}, async() => {
        await this.props.onCostSrvEntranceLoad(true);
      });   
    }
  ...
  componentDidUpdate = async (prevProps) => {
    ...
    if (this.props.costservices_id !== prevProps.costservices_id) {      
      ...
      if (this.props.onCostSrvEntranceLoad !== undefined) {
        this.props.onCostSrvEntranceLoad(true);
      }
  ...
  addRow = async () => {
    ...
    this.var.formMode = 1;
    this.var.formTitle = 'New entrance cost for ' + agentObj.orgService;
  ...
  saveFormData = async () => {
    ...
    condition += (this.var.formMode === 2) ? "AND costservicesentrancefees_id <> " + this.var.formData.costservicesentrancefees_id: "";
    ...
    if (this.var.formMode === 1) {
      ...
      //await this.state.dataGridRef.current.instance.navigateToRow(this.var.formData.ContactSubCategories_id);
  ...
  onFocusedRowChanged = async (e) => {
    this.setState({costservicesentrancefees_id: e.row.data.costservicesentrancefees_id});
    if (this.props.onChangeEntranceRow !== undefined) {
      await this.props.onChangeEntranceRow(e);
    }
  }
  ...
  renderContent() {
    ...
    const dataObj = {
      ...
      canAddRow: false,
      //canDeleteRow: false,
      addRowText: 'Add an Entrance Cost',      
    ...
    const srvEntranceProps = {
      numButtons: 1,
      buttonListObj: [
      {visible: canAdd, options: {icon: "add", onClick: this.addRow, hint: 'Add a new Entrance cost'}}
      ],
      height: 40
    };
    ...
    return (
      <div style={{width: '100%', height: '100%'}}>
        <ToolbarOptions text={"Entrance Costs"} {...srvEntranceProps} ></ToolbarOptions>
...
export default connect(mapStateToProps,mapDispatchToProps)(withRouter(CostSrvEntrance));
