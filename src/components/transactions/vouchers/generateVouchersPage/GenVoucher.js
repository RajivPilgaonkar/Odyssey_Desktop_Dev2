...
function GenVoucher(props) {
  ...
  const displayVoucherSelection = () => {
    _g_history.push({
      pathname: '/VoucherSelection',
      state: {auth: true, backRoute: true}
    });  
  ...
  const displayUncodedVoucherListing = () => {
    _g_history.push({
      pathname: '/ListVoucher',
      state: {auth: true, backRoute: true, uncoded: true}
    });  
  }
